# Authors and Citation

## Authors

- **Athanasia Mo Mowinckel**. Author, maintainer.
  [](https://orcid.org/0000-0002-5756-0223)

- **Didac Vidal-Piñeiro**. Author.
  [](https://orcid.org/0000-0001-9997-9156)

## Citation

Source:
[`DESCRIPTION`](https://github.com/ggsegverse/ggsegGlasser/blob/main/DESCRIPTION)

Mowinckel A, Vidal-Piñeiro D (2026). *ggsegGlasser: Glasser Atlas for
the 'ggsegverse' Ecosystem*. R package version 2.0.4,
<https://github.com/ggsegverse/ggsegGlasser>.

    @Manual{,
      title = {ggsegGlasser: Glasser Atlas for the 'ggsegverse' Ecosystem},
      author = {Athanasia Mo Mowinckel and Didac Vidal-Piñeiro},
      year = {2026},
      note = {R package version 2.0.4},
      url = {https://github.com/ggsegverse/ggsegGlasser},
    }
